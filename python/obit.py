...
def obituary():
    last_user_file = None
    while True:
        user_files = sorted([file for file in os.listdir('/root/Zomboid/Logs/') if file.startswith('user')], reverse=True)
        current_user_file = user_files[0] if user_files else None

        if last_user_file != current_user_file:
            if current_user_file:
                with open(f'/root/Zomboid/Logs/{current_user_file}', 'r') as file:
                    for line in file:
...
def validate():
    while True:
        user_files = [file for file in os.listdir('/root/Zomboid/Logs/') if file.startswith('user')]
        if not user_files:
            time.sleep(1)
        else:
            obituary()
